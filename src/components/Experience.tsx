    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative">
          <div className="absolute left-1/2 transform -translate-x-1/2 w-1 h-full bg-gradient-to-b from-blue-600 to-cyan-600 hidden md:block"></div>
            ...
            {experiences.map((exp, index) => {
              ...
              return (
                ...
                >
                  <div className={`flex-1 ${isEven ? 'md:text-right' : 'md:text-left'}`}>
                    ...
                    >
                      <div className={`flex items-center gap-3 mb-3 ${isEven ? 'md:justify-end' : 'md:justify-start'}`}>
                        <div className={`w-12 h-12 rounded-lg bg-gradient-to-br ${exp.color} flex items-center justify-center ${isEven ? 'order-2' : 'order-1'}`}>
                      ...
                      <p className="text-lg font-semibold text-blue-600 dark:text-cyan-400 mb-2">
                        {exp.organization}
                      </p>
                      ...
                      <ul className={`space-y-2 ${isEven ? 'md:text-right' : 'md:text-left'}`}>
                        {exp.description.map((item, i) => (
                          <li key={i} className="text-gray-700 dark:text-gray-300 text-sm flex items-start gap-2">
                            <span className={`${isEven ? 'order-2' : 'order-1'} flex-1`}>{item}</span>
                            <span className={`w-1.5 h-1.5 rounded-full bg-gradient-to-r ${exp.color} mt-2 ${isEven ? 'order-1' : 'order-2'}`}></span>
                          </li>
                        ))}
                      </ul>
                    </motion.div>
                  ...
                  <div className="hidden md:flex w-8 h-8 rounded-full bg-gradient-to-br from-blue-600 to-cyan-600 items-center justify-center shadow-lg z-10">
        ...
        <motion.div
          ...
          className="mt-16 bg-gradient-to-r from-blue-600 to-cyan-600 rounded-xl p-8 text-white text-center"
        >
